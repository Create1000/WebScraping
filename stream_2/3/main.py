...
def use_re():
    dates = re.findall(r'\b(?:\d{2}/\d{2}/\d{4}|\d{2}-\d{2}-\d{4}|\d{4}[./]\d{2}[./]\d{2}|[A-Z][a-z]+ \d{1,2}, \d{4})\b', text)
    print(dates)
...
def parse_xml(xml_content):
    ...
    # search_field
    search_input = root.find(".//input[@id='text-input-what']")
    print(f"Search Input ID: {search_input.get('id')}, Placeholder: {search_input.get('placeholder')}")

    # search_region
    region_input = root.find(".//input[@id='text-input-where']")
    print(f"Region Input ID: {region_input.get('id')}, Placeholder: {region_input.get('placeholder')}")

    # search_button
    search_button = root.find(".//button[@type='submit']")
    print(f"Search Button Class: {search_button.get('class')}, Text: {search_button.text}")
...
<root>
    <div id="search-container">
        ...
        <button type="submit" class="yosegi-InlineWhatWhere-primaryButton">Search</button>
    </div>
</root>
"""
